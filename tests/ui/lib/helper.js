 ...
 * @return {function} Coverage collection function
 */
function setupCoverage () {
  var hodman, PageObject, originalNavigate, collectCoverage;

  hodman = require('hodman');
  PageObject = hodman.PageObject;
  originalNavigate = PageObject.navigate;

  // Get coverage collection function that collect remote coverage reports
  collectCoverage = global.PRECEPTOR_WEBDRIVER.collectCoverage;

  // Overwrite navigate method on all page-objects to
  // automatically collect coverage reports before moving to new page
  PageObject.navigate = function () {
    collectCoverage();
...
function setupApplication () {
  ...
  application = loadPageObject('application').navigate();
 ...
 */
function setup () {
  var collectCoverage, application, config;

  config = loadConfig();

  setupPageObjects();
  collectCoverage = setupCoverage();
